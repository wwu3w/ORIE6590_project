...
class CityReal(gym.Env):
    ...
    def __init__(self, R, tau_d, L, time_horizon, arrival_rate, trip_dest_prob, travel_time, c_state, capacity=1000):
        ...
        self.c_state = deepcopy(
            self.starting_c_state)  # car state R * tau_d
        self.p_state = np.zeros(
            [R, R])  # passenger_state R * R
        self.It = 0
        self.i = 0
        ...
        for _ in range(self.R * (
                self.tau_d + self.L)):  # car_state
            space_dim.append(int(capacity))
        for _ in range(
                self.R * self.R):  # passenger_state
            space_dim.append(int(capacity))
